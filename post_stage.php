...
if(isset($_POST['create_post'])) {
	...
	$post_teamname = escape($_POST['post_teamname']);
	...
	if($post_title == '' or $post_content == '' or $post_teamname == '') {
	...
	if(empty($error)) {
		...
		if($post_image != NULL) {
			...
			if(!in_array($ext, $allowed_ext)) {
				header("Location: post.php?board=stage&error=imgext");
				exit;
			} else {
				// 画像が回転されている時の修正
		...
		$post_query = "INSERT INTO posts_stage(post_category, post_title, post_teamname, post_author, post_date, post_content, post_file, post_image) ";
		$post_query .= "VALUES('{$post_category}', '{$post_title}', '{$post_teamname}', '{$post_author}', '{$post_date}', '{$post_content}', '{$post_file}', '{$post_image}') ";
						...
						<div class="card mb-5">
							...
							<div class="card-body">
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" name="post_teamname" placeholder="例) HAL Revolution's" autocomplete="off">
								</div>
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" name="post_title" placeholder="例) みんなのプロジェクト" autocomplete="off">
								</div>
								...
								<div class="modal-dialog" role="document">
									<div class="modal-content">
										<div class="modal-header">
											<h5 class="modal-title" id="postCheck">作品公開</h5>
										...
										<div class="modal-footer">
											...
											<button type="submit" name="create_post" class="btn btn-primary">公開する</button>
										</div>
									</div>
								</div>
							</div>

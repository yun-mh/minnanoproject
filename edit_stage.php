...
$sel_query = "SELECT post_author FROM posts_stage WHERE post_id = {$the_post_id} ";
$sel_query_res = mysqli_query($connection, $sel_query);
$res = mysqli_fetch_assoc($sel_query_res);

if($res['post_author'] != $_SESSION['user_id']) {
	header("Location: page.php?board=stage&p_cat=0&page=1");
	exit;
}

$select_query = "SELECT * FROM posts_stage WHERE post_id = $the_post_id ";
...
if(isset($_POST['update_post'])) {
	...
	$update_post_teamname = escape($_POST['post_teamname']);
  ...
  $update_post_image_temp = $_FILES['image']['tmp_name'];
	$update_post_file = $_FILES['file']['name'];
	$update_post_file_temp = $_FILES['file']['tmp_name'];
	
	$error = [	
		'blank'=>''	
	];
	
	if($update_post_title == '' or $update_post_content == '' or $update_post_teamname == '') {
	...
	if(empty($error)) {
		...
		if($update_post_image != NULL) {
			...
			if(!in_array($ext, $allowed_ext)) {
				header("Location: edit.php?board=stage&p_id=$the_post_id&error=imgext");
				exit;
			} else {
				// 画像が回転されている時の修正
		...
		if($post_file != NULL) {
			$update_post_file = time() . $post_author . $update_post_file;
			move_uploaded_file($update_post_file_temp, "./files/files/$update_post_file");
		}

		$update_query = "UPDATE posts_stage SET ";
		$update_query .= "post_title = '{$update_post_title}', ";
		$update_query .= "post_category = '{$update_post_category}', ";
		$update_query .= "post_teamname = '{$update_post_teamname}', ";
		...
		if($update_post_image != NULL) {
			$up_img_query = "UPDATE posts_stage SET ";
		...
		if($update_post_file != NULL) {
			$up_file_query = "UPDATE posts_stage SET ";
			$up_file_query .= "post_file = '{$update_post_file}' ";
			$up_file_query .= "WHERE post_id = $the_post_id ";

			$up_file = mysqli_query($connection, $up_file_query);
		}
						...
						<div class="card mb-5">
							...
							<div class="card-body">
								...
								<div class="form-group">
									<input type="text" readonly class="form-control-plaintext" name="post_author" value="<?php echo $_SESSION['user_id'];?>" hidden>
								</div>
								
								<div class="form-group">
									<label for="title">チーム名</label>
									<input type="text" class="form-control" name="post_teamname" value="<?php echo $post_teamname?>" placeholder="例) HAL Revolution's" autocomplete="off">
								</div>

								<div class="form-group">
									<label for="category">作品分野</label>
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" name="post_title" value="<?php echo $post_title; ?>"  placeholder="例) みんなのプロジェクト" autocomplete="off">
								</div>

								<div class="form-group">
									<label for="post_content">内容</label>
									<textarea class="form-control" name="post_content" id="body" rows="10" placeholder="例) みんなのプロジェクトを公開します！
皆からのコメントやアップを期待していますので、ぜひフィードバックしてください！									
								...
								<div class="form-group">
									<label for="post_file">ファイル</label>
									<div class="card col-md-3 px-0">
										<div class="card-header">
											前にアップロードしたファイル
										</div>
										<div class="card-body text-center">
											<?php
											if($post_file != NULL) {
												echo "<a href='files/files/$post_file'>{$post_file}</a>";		
											}
											?>

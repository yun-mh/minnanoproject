...
$sel_query = "SELECT post_author FROM posts_project WHERE post_id = {$the_post_id} ";
...
$select_query = "SELECT * FROM posts_project WHERE post_id = $the_post_id ";
...
if(isset($_POST['update_post'])) {
  ...
  $update_post_image_temp = $_FILES['image']['tmp_name'];
	...
	if(empty($error)) {
		...
		$update_query = "UPDATE posts_project SET ";
		...
		if($update_post_image != NULL) {
			$up_img_query = "UPDATE posts_project SET ";
						...
						<div class="card mb-5">
							...
							<div class="card-body">
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" name="post_title" value="<?php echo $post_title; ?>" placeholder="例)「OOOプロジェクト」" autocomplete="off">
								</div>

								<div class="form-group">
									<label for="password">ルームパスワード(修正不可)</label>
									<input type="password" readonly id="roompw" class="form-control" name="post_password" placeholder="数字4桁" value="<?php echo $post_password?>">
								</div>
								...
								<div class="modal-dialog" role="document">
									<div class="modal-content">
										<div class="modal-header">
											<h5 class="modal-title" id="postCheck">プロジェクトルーム修正</h5>

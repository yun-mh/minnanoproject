...
if(isset($_GET['p_id'])) {
	...
	$view_query = "UPDATE posts_notice SET post_views_count = post_views_count + 1 WHERE post_id = $the_post_id ";
	$send_view_query = mysqli_query($connection, $view_query);
	
}

$read_query = "SELECT * FROM posts_notice WHERE post_id = $the_post_id ";
			...
			<article>
				...
				<div id="post_tool">
					<nav class="navbar navbar-light bg-light">
						<div class="container">　　
							<div id='list_btn'>
								<input type='button' class='btn btn-outline-secondary' value='リストへ' onclick="location.href='page.php?board=notice&p_cat=0&page=1'">
							</div>	
						</div>
					</nav>
				</div>
							...
							<div class="row">
								...
								<div class="col-2">
										...
										if($_SESSION['user_auth'] == 'admin') {
											echo "<a href='edit.php?board=notice&p_id={$post_id}' class='btn btn-outline-warning'>修正</a>";
										}
										?>
								</div>
								<div class="col-2">
						...
						<div class="card-body">
							...
							<div class="card-footer bg-light py-0 px-0">
								...
								<div class="card-body py-2">
									...
									$cmt_query_count = "SELECT * FROM comments_notice WHERE comment_post_id = $post_id ";
									...
									$show_cmt_query = "SELECT * FROM comments_notice WHERE comment_post_id = {$the_post_id} ORDER BY comment_id DESC LIMIT $s_point, $list ";
						...
						<?php	} ?>
									...
									<div id="page_box">
										<nav>
											...
											?>
												...
												<li class="page-item"><a class="page-link" href="view.php?board=notice&p_id=<?=$post_id?>&cmt=<?=$s_page-1?>">前へ</a></li>
											...
											?>
												...
												for($i=$s_page; $i<=$e_page; $i++) {
													...
													if($i == $page) {
														echo "<li class='page-item active'><a class='page-link' href='view.php?board=notice&p_id=$post_id&cmt={$i}'>{$i}</a></li>";
													} else {
														echo "<li class='page-item'><a class='page-link' href='view.php?board=notice&p_id=$post_id&cmt={$i}'>{$i}</a></li>";
													}	
												}
												...
												<li class="page-item"><a class="page-link" href="view.php?board=notice&p_id=<?=$post_id?>&cmt=<?=$e_page+1?>">次へ</a></li>
							...
							if(isset($_POST['create_comment'])) {
								...
								if($comment_content == '') {									
									...
									$cmt_query = "INSERT INTO comments_notice (comment_post_id, comment_author, comment_content, comment_date) ";
									...
									$cmt_num_query = "UPDATE posts_notice SET post_comment_count = post_comment_count + 1 ";
									$cmt_num_query .= "WHERE post_id = $the_post_id ";

									$update_cmt_num = mysqli_query($connection, $cmt_num_query);

									header("Location: view.php?board=notice&p_id=$the_post_id&cmt=1");
								}
							}
 		...
 		<script>
			...
			$(document).ready(function(){
				...
				$('.delete_link').on('click', function(){
					...
					var delete_url = "page.php?board=notice&p_cat=0&page=1&delete="+ id +" ";

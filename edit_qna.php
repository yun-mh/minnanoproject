...
$sel_query = "SELECT post_author FROM posts_qna WHERE post_id = {$the_post_id} ";
...
$select_query = "SELECT * FROM posts_qna WHERE post_id = $the_post_id ";
	...
	if(empty($error)) {
		...
		if($update_post_image != NULL) {
			...
			if(!in_array($ext, $allowed_ext)) {
				header("Location: edit.php?board=qna&p_id=$the_post_id&error=imgext");
				exit;
			} else {
				// 画像が回転されている時の修正
		...
		$update_query = "UPDATE posts_qna SET ";
		...
		if($update_post_image != NULL) {
			$up_img_query = "UPDATE posts_qna SET ";
		...
		if($update_post_file != NULL) {
			$up_file_query = "UPDATE posts_qna SET ";
						...
						<div class="card mb-5">
							...
							<div class="card-body">
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" name="post_title" value="<?php echo $post_title; ?>" placeholder="例) 配列って一体何ですか？" autocomplete="off">
								</div>
								...
								<div class="modal-dialog" role="document">
									<div class="modal-content">
										<div class="modal-header">
											<h5 class="modal-title" id="postCheck">質問修正</h5>

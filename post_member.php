	...
	if(empty($error)) {
		...
		if($post_image != NULL) {
			...
			if(!in_array($ext, $allowed_ext)) {
				header("Location: post.php?board=member&error=imgext");
				exit;
			} else {
				// 画像が回転されている時の修正
		...
		$post_query = "INSERT INTO posts_member(post_category, post_title, post_author, post_date, post_content, post_image) ";
		$post_query .= "VALUES('{$post_category}', '{$post_title}', '{$post_author}', '{$post_date}', '{$post_content}', '{$post_image}') ";
						...
						<div class="card mb-5">
							...
							<div class="card-body">
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" name="post_title" placeholder="例)「OOOプロジェクト」ロゴ制作者募集" autocomplete="off">
								</div>
								...
								<div class="modal-dialog" role="document">
									<div class="modal-content">
										<div class="modal-header">
											<h5 class="modal-title" id="postCheck">メンバー募集投稿</h5>

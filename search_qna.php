			...
			<article>
					...
					<nav class="navbar navbar-light bg-light">
						<div class="container">
						  ...
						  <!-- 検索フォーム -->
							...
							<div id='list_btn'>
								<input type='button' class='btn btn-outline-secondary' value='全体リストへ' onclick="location.href='page.php?board=qna&p_cat=0&page=1'">
							</div>
				...
				<div id="container" class="container">
						...
						<?php 
								...
								echo "<div>
												<table id='qna' class='table table-sm table-bordered table-hover'>
													<thead>
														<tr>
															<th>項番</th>
															<th>分類</th>
								...
								if(isset($_GET['search_content'])) {
									...
									$src_query = "SELECT * FROM posts_qna WHERE post_title LIKE '%$search%' OR post_content LIKE '%$search%' OR post_author LIKE '$search' ";
									...
									$query = "SELECT * FROM posts_qna WHERE post_title LIKE '%$search%' OR post_content LIKE '%$search%' OR post_author LIKE '%$search%' ORDER BY post_id DESC LIMIT $s_point, $list";
									...
									if($num == 0) {
										...
										while($row = mysqli_fetch_assoc($select_posts)) {
											...
											echo "<td><a href='view.php?board=qna&p_id=$post_id&cmt=1'>$post_title</a></td>";
						...
						?>
							...
							<div class="row justify-content-center">
								...
								<div>
									<a class='btn btn-outline-primary mx-3' href='search.php?board=qna&search_content=<?=$search?>&page=1'>最初へ</a>
								</div>
								...
								<div id="page_box">
									<nav>
										<ul class="pagination">
											...
											<li class="page-item"><a class="page-link" href="search.php?board=qna&search_content=<?=$search?>&page=<?=$s_page-1?>">前へ</a></li>
											...
											for($i=$s_page; $i<=$e_page; $i++) {
												...
												if($i == $page) {
													echo "<li class='page-item active'><a class='page-link' href='search.php?board=qna&search_content={$search}&page={$i}'>{$i}</a></li>";
												} else {
													echo "<li class='page-item'><a class='page-link' href='search.php?board=qna&search_content={$search}&page={$i}'>{$i}</a></li>";
												}	
											}
											...
											<li class="page-item"><a class="page-link" href="search.php?board=qna&search_content=<?=$search?>&page=<?=$e_page+1?>">次へ</a></li>
								...
								<div>
									<a class='btn btn-outline-primary mx-3' href='search.php?board=qna&search_content=<?=$search?>&page=<?=$page_num?>'>最後へ</a>
								</div>
